...
(function () {
    ...
    const color = encodeURIComponent(script.dataset.primaryColor || '#f15b2f');
    ...
    function applyStyle(expanded) {
        ...
        if (expanded) {
            iframe.style.cssText = isMobile
                ? `position:fixed;bottom:20;right:20;width:100vw;height:100dvh;border:none;z-index:999999;border-radius:0;transition:all 0.1s ease;background:transparent;`
                : `position:fixed;bottom:20px;right:20px;width:${PANEL_W};height:${PANEL_H};border:none;border-radius:16px;box-shadow:0 20px 60px rgba(0,0,0,0.15);z-index:999999;transition:all 0.1s ease;background:transparent;`;
        } else {
            iframe.style.cssText = `position:fixed;bottom:20px;right:20px;width:${BUBBLE_SIZE};height:${BUBBLE_SIZE};border:none;border-radius:9999px;z-index:999999;background:transparent;transition:all 0.1s ease;`;
        }
    }
